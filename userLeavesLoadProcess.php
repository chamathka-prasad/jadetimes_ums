...
if (isset($_SESSION["jd_admin"])) {
    ...
    if (!empty($from)) {
        ...
        if (empty($to)) {
            if (empty($typeFilter)) {
                $typeFilter = "WHERE `leave`.`leave_date` ='" . $from . "'";
            } else {
                $typeFilter = $typeFilter . " AND `leave`.`leave_date` = '" . $from . "'";
            }
        } else {
            if (empty($typeFilter)) {
                $typeFilter = "WHERE `leave`.`leave_date` >= '" . $from . "' AND `leave`.`leave_date` <= '" . $to . "'";
            } else {
    ...
    if (!empty($status)) {
        ...
        if (empty($typeFilter)) {
            if ($status != 0) {
                $typeFilter = "WHERE `leave`.`leave_status_id` ='" . $status . "'";
            }
        } else {
            if ($status != 0) {
    ...
    $userResult = Database::operation("SELECT user.fname,user.lname,user.email,user.jid,leave.leave_date,leave.leave_status_id,leave.id  FROM `leave`
     INNER JOIN `user` ON `user`.`id`=`leave`.`user_id` " . $typeFilter . " LIMIT 25 OFFSET " . $offset, "s");
