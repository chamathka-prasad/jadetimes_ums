...
if (isset($_SESSION["jd_admin"])) {
    ...
    if ($results->num_rows > 0) {
        ...
        $resultsLeave = Database::operation("SELECT leave_date,leave_status_id FROM `leave` WHERE `user_id`='" . $userSession["id"] . "' AND `leave_date`>='" . date("Y-m-d", $timeGap) . "' AND `leave_status_id` IN('2','4')", "s");
