...
INNER JOIN `position` ON `position`.`id`=`user`.`position_id` INNER JOIN `department` ON `department`.`id`=`position`.`department_id` LEFT JOIN `profile_image` ON `profile_image`.`user_id`=`user`.`id` INNER JOIN `pending_attendance` ON `pending_attendance`.`user_id`=`user`.`id`  WHERE `department`.`name`='" . $_SESSION["jd_user"]["department"] . "' AND `pending_attendance`.`status`='1'  ORDER BY `user`.`user_status_id` ASC", "s");
				...
				fetch("attendanceapproveProcess.php?pid=" + id, {
					...
					.then(function(value) {
						...
						if (value.type == "error") {
							...
							setTimeout(() => {
								window.location = "pendingAttendanceView.php";
							}, 1000);
						} else {
							msg.classList = "alert alert-danger";
							msg.innerHTML = "Something wrong please try again";
