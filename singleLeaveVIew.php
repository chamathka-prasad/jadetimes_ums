...
INNER JOIN `position` ON `position`.`id`=`user`.`position_id` INNER JOIN `department` ON `department`.`id`=`position`.`department_id` LEFT JOIN `profile_image` ON `profile_image`.`user_id`=`user`.`id`  WHERE `leave`.`id`='" . $leaveId . "';", "s");
...
?>
		<script>
			window.location = "manageLeaves.php"
		</script>
	<?php
	}
	?>
				...
				<div class="app-container">
					...
					<div class="app-hero-header d-flex align-items-start">
						...
						<ol class="breadcrumb d-none d-lg-flex align-items-center">
							...
							<li class="breadcrumb-item"><a href="manageLeaves.php">Manage Leaves</a></li>
										...
										<div class="col-12">
											<div class="row">
												...
												<div class="col-lg-5 col-sm-8 col-8">
													<div class="mb-3">
														<label class="form-label">Leave Status</label><label class="colorRed ms-3 fs-6">*</label>
														<select class="form-select removeCorner" id="leave_state">
															...
															$leaveStatresult = Database::operation("SELECT * FROM `leave_status`", "s");
															$unum = $leaveStatresult->num_rows;
															if ($unum != 0) {
																for ($i = 0; $i < $unum; $i++) {

																	$leave = $leaveStatresult->fetch_assoc();
																	$leaveIds = $leave["id"];

																
															?>
																		<option value="<?php echo $leaveIds ?>" <?php if ($leaveIds == $Profile["leave_status_id"]) { ?> selected <?php } ?>><?php echo $leave["name"] ?></option>
															<?php
												...
												<div class="col-lg-3 mt-4 col-sm-4 col-4">
													<div class="d-flex mt-1 gap-2 justify-content-center">

														<button type="button" onclick='updateLeaveStatus(<?php echo $leaveId ?>)' class="btn btn-primary removeCorner backgroundColorChange">
															Update Status
														</button>
													</div>
												</div>
									...
									<div class="col-12">
										<div class="card">
											<div class="card-body">
												...
												} else if($Profile["leave_status_id"] == 4) {
												?>
													<div class="alert alert-warning" role="alert">
														<div class="mb-3">
														<i class="bi bi-exclamation-octagon-fill fs-1 me-2 lh-1"></i>
														</div>
														<h4 class="alert-heading">Emergency Leave !</h4>
														<p>
															Leave Date -> <?php echo $Profile["leave_date"] ?>
														</p>
														<hr />
												...
												?>
													<div class="alert alert-primary" role="alert">
														...
														<h4 class="alert-heading">Special Leave !</h4>
														<p>
															Leave Date -> <?php echo $Profile["leave_date"] ?>
														</p>
														<hr />
