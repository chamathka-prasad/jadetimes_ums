    ...
    $regex = '/^\+?\d+$/';
     ...
     if (empty($line1)) {
        ...
        if ($Forward) {
            ...
            $addressResults = Database::operation("SELECT * FROM `address` WHERE `address`.`user_id`='" . $sessionAdmin["id"] . "'", "s");
            if ($addressResults->num_rows == 1) {

                Database::operation("UPDATE `address` SET `address`.`no`='" . $no . "',`address`.`line1`='" . $line1 . "',`address`.`line2`='" . $line2 . "',`address`.`city`='" . $city . "',`address`.`country_id`='" . $countryId . "' WHERE `address`.`user_id`='" . $sessionAdmin["id"] . "'", "iud");
            } else {
                Database::operation("INSERT INTO `address`(`no`,`line1`,`line2`,`city`,`country_id`,`user_id`)VALUES('" . $no . "','" . $line1 . "','" . $line2 . "','" . $city . "','" . $countryId . "','" . $sessionAdmin["id"] . "')", "iud");
            }
            ...
            if (isset($_FILES["img"])) {
                ...
                $profileImgResult = Database::operation("SELECT * FROM `profile_image` WHERE `user_id`='" . $sessionAdmin["id"] . "'", "s");
                if ($profileImgResult->num_rows == 1) {
                    Database::operation("UPDATE `profile_image` SET `profile_image`.`name`='" . $savePath . "' WHERE `user_id`='" . $sessionAdmin["id"] . "'", "iud");
                } else {

                    Database::operation("INSERT INTO `profile_image`(`name`,`user_id`)VALUES('" . $savePath . "','" . $sessionAdmin["id"] . "')", "iud");
                }
                move_uploaded_file($img["tmp_name"], $path);
            }

            $results = Database::operation("SELECT `user`.`id`,`user`.`email`,`user`.`password`,`user`.`fname`,`user`.`lname`,`user`.`jid`,`type`.`name` AS `user_type`,`position`.`name` AS `position`,`department`.`name` AS `department`,`profile_image`.`name` AS `imgPath` FROM `user` INNER JOIN `type` ON  `type`.`id`=`user`.`type_id` INNER JOIN `user_status` ON `user`.`user_status_id`=`user_status`.`id` INNER JOIN `position` ON `position`.`id`=`user`.`position_id` INNER JOIN `department` ON `department`.`id`=`position`.`department_id` LEFT JOIN  `profile_image` ON `profile_image`.`user_id`=`user`.`id` WHERE `user`.`id`='" . $sessionAdmin["id"] . "' AND `user_status`.`name`='ACTIVE'", "s");
