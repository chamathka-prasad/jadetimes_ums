...
if (isset($_SESSION["jd_admin"])) {
    ...
    if (!empty($from)) {
        ...
        if (empty($to)) {
            if (empty($typeFilter)) {
                $typeFilter = "WHERE `attendance`.`date_time` LIKE '" . $from . "%'";
            } else {
                $typeFilter = $typeFilter . " AND `attendance`.`date_time` LIKE '" . $from . "%'";
            }
        } else {
            if (empty($typeFilter)) {
                $typeFilter = "WHERE `attendance`.`date_time` >= '" . $from . "' AND `attendance`.`date_time` <= '" . $to . "'";
            } else {
    ...
    if ($mark != 2) {
        ...
        $userResultSearch = Database::operation("SELECT user.fname,user.lname,user.email,user.mobile,user.jid,user.reg_date,user.id FROM `user` WHERE `user`.`user_status_id`='1' AND `user`.`type_id`!='3' " . $userIdSort . "
        LIMIT 25 OFFSET " . $offset, "s");

        $userResultSearchCount = Database::operation("SELECT COUNT(*) AS total_rows FROM `user` WHERE `user`.`user_status_id`='1' AND `user`.`type_id`!='3' " . $userIdSort . "", "s");
...
INNER JOIN  `attendance`  ON `user`.`id`=`attendance`.`user_id` WHERE `attendance`.`date_time` LIKE '" . $from . "%' " . $userIdSort . "", "s");
     ...
     INNER JOIN `user` ON `user`.`id`=`attendance`.`user_id` " . $typeFilter . " LIMIT 25 OFFSET " . $offset, "s");
